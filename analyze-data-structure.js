...
async function analyzeDataStructure() {
    ...
    if (result && result.data) {
      ...
      if (data.length > 0) {
        ...
        if (ep.name === 'Mining Sites') {
          data.forEach((site) => {
            console.log(`   - ${site.code}: ${site.name} (Type: ${site.siteType})`);
          });
        }

        if (ep.name === 'Road Segments') {
          data.slice(0, 5).forEach((road) => {
            console.log(`   - ${road.code}: ${road.name} | Distance: ${road.distance}km | Condition: ${road.roadCondition} | Mining Site: ${road.miningSiteId}`);
          });
        }

        if (ep.name === 'Weather Logs') {
          data.slice(0, 3).forEach((w) => {
            console.log(`   - Site: ${w.miningSite?.name || w.miningSiteId} | Condition: ${w.condition} | Temp: ${w.temperature}°C`);
          });
        }
        ...
        if (ep.name === 'Sailing Schedules') {
          data.slice(0, 5).forEach((s) => {
            console.log(`   - ID: ${s.id} | Vessel: ${s.vessel?.name || s.vesselId} | ETS: ${s.etsLoading} | Qty: ${s.plannedQuantity}T`);
          });
        }
      }
    }
  ...
  if (dataMap['Mining Sites'] && dataMap['Weather Logs']) {
    ...
    dataMap['Weather Logs'].forEach((w) => {
      ...
      if (siteId) {
        ...
        siteWeather[siteId].push(w.condition);
      }
    });

    dataMap['Mining Sites'].forEach((site) => {
      const weatherCount = siteWeather[site.id]?.length || 0;
      const latestWeather = siteWeather[site.id]?.[0] || 'N/A';
      console.log(`   ${site.name}: ${weatherCount} weather records, latest: ${latestWeather}`);
    });
  }
  ...
  if (dataMap['Road Segments'] && dataMap['Mining Sites']) {
    ...
    dataMap['Road Segments'].forEach((r) => {
      ...
      siteRoads[siteId].push({ code: r.code, condition: r.roadCondition, distance: r.distance });
    });
    ...
    dataMap['Mining Sites'].forEach((site) => {
      ...
      roads.slice(0, 2).forEach((r) => {
        console.log(`      - ${r.code}: ${r.distance}km, Condition: ${r.condition}`);
      });
    });
  }
